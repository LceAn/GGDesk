...
def save_config(config):
    ...
    try:
        ...
        print(f"警告：无法保存配置 {CONFIG_FILE}: {e}")


# --- 5. 核心扫描逻辑 (Beta 3.0 支持中断) ---
...
def discover_programs(scan_path, blocklist, log_callback, check_stop_callback=None):
    ...
    v7 核心算法：程序发现
    - log_callback: 用于输出日志
    - check_stop_callback: 【Beta 3 新增】一个函数，如果返回 True，则停止扫描
    """
    ...
    for root, dirs, files in os.walk(scan_path, topdown=True):
        ...
        for file in files:
            if file.lower().endswith(".exe"):
                if file.lower() in blocklist:
                    # 为了减少日志刷屏，过滤信息可以只在调试模式显示，或者简化显示
                    # log_callback(f"[过滤] {file}")
                    continue
        ...
        if current_exes:
            ...
            # 实时反馈发现进度（可选）
            # log_callback(f"扫描中... 发现 {len(current_exes)} 个程序在 {os.path.basename(root)}")
    ...
    log_callback(f"初步整理: 发现 {len(top_level_folders)} 个顶层目录，正在进行智能分组...")
    ...
    log_callback(f"--- 发现完成 ---")
    log_callback(f"总共发现 {len(final_programs)} 个程序。")
