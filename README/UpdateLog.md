## 📝 完整更新日志
### [2025-11-24] Beta 10.0 - 文件结构重构

**📂 目录结构优化 (Directory Organization)**

  * **Config Isolation**: 所有配置文件（`.ini`, `.txt` 规则列表）现在统一存储在 **`config/`** 子目录下，不再污染根目录。
  * **Data Separation**: 数据库文件（`.db`）统一存储在 **`data/`** 子目录下。

**🔄 自动迁移 (Auto-Migration)**

  * **Smart Init**: 程序启动时会自动检测根目录是否存在旧版本的配置文件或数据库。
  * **Seamless Transition**: 如果发现旧文件，程序会自动将其**移动**到对应的新文件夹中，确保用户在升级版本后数据不丢失、配置不重置。

---
### [2025-11-22] Beta 9.9 - 列表设置归位

**🖥️ 界面逻辑优化 (UI Logic)**

  * **设置归位 (Relocation)**:
      * 将“扫描结果默认勾选策略”从全局规则弹窗中移除。
      * 在扫描页面的结果列表工具条中新增 **“⚙️ 列表设置”** 菜单。
  * **上下文一致性**: 用户现在可以直接在查看结果时调整“默认勾选新增/已存在”的策略，无需跳转到二级弹窗，大大提升了操作的连贯性。
  * **实时生效**: 通过菜单调整的策略会立即写入配置文件，确保下一次扫描或新发现的项目即时应用新规则。
---
### [2025-11-22] Beta 9.7 - 规则美化与编程环境过滤

**🛡️ 过滤能力增强 (Filtering)**

  * **编程环境过滤 (Prog Filter)**:
      * 新增 `DEFAULT_PROG_RUNTIMES` 词库，包含 `python.exe`, `java.exe`, `node.exe`, `gcc.exe` 等常见解释器和编译器。
      * 在规则管理页新增 **“🚫 过滤编程运行环境”** 开关（默认开启）。此功能可防止扫描器误将开发环境的底层二进制文件识别为用户应用程序。

**🎨 规则窗口美化 (Rules UI)**

  * **Grid 布局**: 彻底重构了“目标文件类型”区域，采用 2x2 网格布局，解决了旧版排列松散、不对齐的问题。
  * **GroupBox 样式**: 为所有分组标题添加了蓝色高亮 QSS 样式，视觉层次更加分明。
  * **逻辑分组**: 将配置项重新梳理为 `1.目标文件`、`2.过滤清洗`、`3.智能策略`、`4.默认行为` 四大板块，操作逻辑更符合直觉。

**🔧 兼容性**

  * **全类型支持**: 确保了 `.bat` 和 `.lnk` 文件类型在后端扫描逻辑中被正确处理，并受统一的黑名单和大小规则约束。
-----
### [2025-11-21] Beta 9.6 - 仪表盘式扫描页 (Dashboard Scanner)

**🖥️ 界面重构 (UI Overhaul)**

  * **控制面板化 (Dashboard Layout)**: 重新设计了扫描页面的布局，将配置、路径、按钮整合为顶部的“控制面板”区域，与下方的“结果列表”区域在视觉上做了明确区分，操作逻辑更清晰。
  * **规则胶囊展示 (Rules Badge)**: 恢复并美化了“当前生效规则”的显示，现在它位于控制面板底部，以简洁的图标文本形式呈现当前启用的过滤条件。

**⚙️ 功能增强 (Enhancements)**

  * **列表过滤器 (Result Filter)**: 在结果列表顶部新增了 **“来源筛选”** 下拉菜单。
      * 支持仅查看 `开始菜单`、`应用商店` 或 `自定义目录` 的结果。
      * 配合全选/反选功能，用户可以极其方便地进行“只生成应用商店快捷方式”等批量操作。
---
### [2025-11-21] Beta 9.5 - 实时流式修复 (Streaming Hotfix)

**⚡ 核心算法重写 (Core Rewrite)**

  * **实时显示回归**: 彻底重写了 `core_discovery.py`。
      * 废弃了导致阻塞的“全量收集-后置去重”逻辑。
      * 采用了 **流式去重 (Streaming Dedup)** 策略，扫描器现在每发现一个程序，会立即进行去重判断并 Yield 给 UI，实现了真正的**实时刷新**。
  * **自定义目录修复**: 修复了“自定义目录”扫描为空的严重 Bug。
      * **原因**: 旧版 `ignored_dirs.txt` 中残留了 `bin` 目录，导致大多数软件的主程序被跳过。
      * **解决**: 在 `manager_rules.py` 中增加了强制清洗逻辑，自动从忽略列表中剔除 `bin`, `lib` 等高频目录。

---
### [2025-11-21] Beta 9.4 - 状态栏重构与进度反馈 (Status Bar Polish)

**✨ 视觉美化 (Visuals)**

  * **Modern Status Bar**: 彻底重写了状态栏样式。去除陈旧的凹陷边框，采用纯平白色背景与顶部细灰线分割，与整体 Fluent 风格保持一致。
  * **Ultra-thin Progress**: 进度条改为 4mm 超薄脉冲样式（蓝色），仅在任务进行时显示，视觉干扰降至最低。

**🐛 逻辑修复 (Fixes)**

  * **Reliable Progress**: 废弃了基于文本匹配的进度条控制逻辑，转而使用专用的 `sig_busy(bool)` 信号。现在无论状态文字如何变化，只要后台任务在运行，进度条就会稳定显示。
  * **Text Stretch**: 修复了长路径文本挤压问题，状态标签现在会自动占据剩余空间，确保信息完整显示。
